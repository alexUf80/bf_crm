...
*/
        ...
        if ($insurance_operations = $this->operations->get_operations(array('type' => 'INSURANCE', 'date_from' => $date_from, 'date_to' => $date_to)))
        {
            ...
            {
                if (!empty($io->contract_is_closed))
                {
                    $insurance_close_count++;
                    $insurance_close_summ += $io->amount;                    
                }
                else
                {
        ...
        $msg .= '</tr>';
        $msg .= '<tr>';
        $msg .= '<td style="padding:6px; width:330; background-color:#f0f0f0; border:1px solid #e0e0e0;">Страховки при закрытии (кол-во/сумма)</td>';
        $msg .= '<td style="padding:6px; width:170; background-color:#ffffff; border:1px solid #e0e0e0;text-align:right;">'.$insurance_close_count.' / '.$insurance_close_summ.'</td>';
        $msg .= '</tr>';
        $msg .= '<tr>';
