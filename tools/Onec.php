        ...
        foreach ($orders as $order) {
            ...
            $xml['Справочники'][$i]['Контрагент'] =
                [
                    ...
                    'РайонПоРегистрации' => trim($order->user->regAddress->district),
                    'РайонФактическогоПроживания' => trim($order->user->factAddress->district),
                    'ГородПоРегистрации' => (trim($order->user->regAddress->city) == '' ? trim($contract->user->regAddress->locality . ' ' . $contract->user->regAddress->locality_type) : trim($contract->user->regAddress->city . ' ' . $contract->user->regAddress->city_type)),
                    'ГородФактическогоПроживания' => (trim($order->user->factAddress->city) == '' ? trim($contract->user->factAddress->locality . ' ' . $contract->user->factAddress->locality_type) : trim($contract->user->factAddress->city . ' ' . $contract->user->factAddress->city_type)),
        ...
        foreach ($contracts as $contract) {
            ...
            $xml['Справочники'][$i]['Контрагент'] =
                [
                    ...
                    'РайонПоРегистрации' => trim($contract->user->regAddress->district),
                    'РайонФактическогоПроживания' => trim($contract->user->factAddress->district),
                    'ГородПоРегистрации' => (trim($contract->user->regAddress->city) == '' ? trim($contract->user->regAddress->locality . ' ' . $contract->user->regAddress->locality_type) : trim($contract->user->regAddress->city . ' ' . $contract->user->regAddress->city_type)),
                    'ГородФактическогоПроживания' => (trim($contract->user->factAddress->city) == '' ? trim($contract->user->factAddress->locality . ' ' . $contract->user->factAddress->locality_type) : trim($contract->user->factAddress->city . ' ' . $contract->user->factAddress->city_type)),
